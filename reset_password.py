...
def build_migrated_catalog(doc, user_lookup, catalog_id_str):
    ...
    # ── Nombre / Descripción ────────────────────────────────────────────────
    name = (
        doc.get("Name") or doc.get("name") or "Sin nombre"
    )
    ...
    if owner_str and len(owner_str) != 24:
        ...
        if resolved:
            ...
            print(f"   ⚠️  No se encontró usuario para owner='{owner_str}' — se deja como string")
...
def fix_existing_row_files(row):
    ...
    for key in ["Images", "images", "Documents", "documents", "MultimediaFiles", "multimediaFiles"]:
        if key in files:
...
def build_user_lookup(users_col):
    ...
    """
    lookup = {}
    for u in users_col.find({}, {"_id": 1, "username": 1, "email": 1, "name": 1,
                                   "Username": 1, "Email": 1, "Name": 1}):
        oid = str_id(u["_id"])
    ...
    for k in sorted(all_keys):
        ...
        if k == "Rows":
            ...
            if old_count != new_count:
                ...
                diffs.append(f"  Rows: misma cantidad ({new_count}) pero diferente estructura")
            continue
...
def main():
    ...
    for doc in catalog_docs:
        ...
        if not DRY_RUN:
            catalogs_col.replace_one({"_id": doc["_id"]}, {"_id": doc["_id"], **{k: v for k, v in new_doc.items() if k != "_id"}})
            print(f"   ✅ Actualizado")
    ...
    for doc in ss_docs:
        ...
        if not DRY_RUN:
            catalogs_col.insert_one({"_id": doc["_id"], **{k: v for k, v in new_doc.items() if k != "_id"}})
            print(f"   ✅ Insertado")
